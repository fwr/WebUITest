...
public class ZTestReport implements IReporter {
    //报告的路径
    private String basePath = System.getProperty("user.dir") + File.separator +"test-output";
    //报告的名称
    private String reportName=File.separator+ "report.html";
    ...
    private void outputResult(List<ITestResult> list) {
        try {
            ...
            FileUtil.createFile(basePath,reportName);
            BufferedWriter output = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(basePath+reportName)), "UTF-8"));
